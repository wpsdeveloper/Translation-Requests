 ...
 * @param {Object} rowData The raw row data from AppSheet.
 */
function sendNewRequestNotification(rowData: any) {
  try {
    Logger.log('Processing new request notification for: ' + JSON.stringify(rowData));

    // 1. Normalize data
    const request = mapAppSheetRequest(rowData);
    ...
    const schoolApprovers = allUsers.filter(user => 
      user.role === 'Approver' && 
      user.schools.includes(request.school)
    );
...
function testNotification() {
  ...
  sendNewRequestNotification(mockRow);
}
