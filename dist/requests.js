...
function getSchoolsFromAppSheet() {
    ...
    const data = JSON.parse(response.getContentText());
    if (data && Array.isArray(data)) {
        return data.map((row) => row.Name || row.name || '');
    }
    return [];
}
/**
 ...
 */
function addRequestToServer(data) {
    const activeUserEmail = Session.getActiveUser().getEmail();
...
function uploadFileToDrive(base64Data, fileName, mimeType) {
    try {
        ...
        throw new Error('Failed to upload file to Google Drive: ' + ((e === null || e === void 0 ? void 0 : e.toString()) || 'Unknown error'));
    }
}
/**
